...
const LoadingCard: React.FC<LoadingCardProps> = ({
  ...
  const renderSkeleton = (index: number) => {
    if (variant === 'bundling') {
      return (
        <div key={index} className="rounded-xl border border-light dark:border-dark bg-white dark:bg-dark/90 shadow-sm h-full flex flex-col relative overflow-hidden">
          {/* Shimmer overlay */}
      ...
      return (
        <div key={index} className="rounded-lg border border-light dark:border-dark bg-white dark:bg-dark/90 p-3 shadow-sm h-full relative overflow-hidden">
          {/* Shimmer overlay */}
          <div className="absolute inset-0 -translate-x-full animate-shimmer bg-gradient-to-r from-transparent via-white/20 dark:via-white/10 to-transparent"></div>
    ...
    } else { // Default product variant
      return (
        <div key={index} className="rounded-xl md:rounded-lg border border-light dark:border-dark bg-white dark:bg-dark/90 p-2 md:p-3 shadow-sm h-full flex flex-col relative overflow-hidden">
          {/* Shimmer overlay */}
          <div className="absolute inset-0 -translate-x-full animate-shimmer bg-gradient-to-r from-transparent via-white/20 dark:via-white/10 to-transparent"></div>
