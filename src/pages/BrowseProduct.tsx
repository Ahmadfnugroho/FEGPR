...
export default function BrowseProduct() {
  ...
  const [bundlingLoading, setBundlingLoading] = useState(true);
  const [bundlingLoadingMore, setBundlingLoadingMore] = useState(false);
  ...
  // Bundling-specific pagination / loading (separate from products)
  const [bundlingPage, setBundlingPage] = useState(1);
  const [bundlingTotalPages, setBundlingTotalPages] = useState(1);
  const [bundlingHasMore, setBundlingHasMore] = useState(true);
  ...
  const noFiltersActive =
    ...
    (!filter.brand || filter.brand.length === 0) &&
  ...
  const bundlingAbortRef = useRef<AbortController | null>(null);
  ...
  useEffect(() => {
    return () => {
      ...
      try {
        bundlingAbortRef.current?.abort();
      } catch (e) {}
    };
  }, []);
    ...
    Promise.all([
      ...
      .catch((e) => {
        ...
        const errorMessage =
          ...
          e.message ||
          "Gagal memuat data kategori, brand, dan sub-kategori";
  ...
  const fetchBundlings = useCallback(
    async (p = 1, append = false) => {
      // Cancel previous bundling request if it exists
      if (bundlingAbortRef.current) {
        console.log("🚫 [BrowseProduct] Aborting previous bundlings request");
        bundlingAbortRef.current.abort("New bundlings request initiated");
      }

      // Create new AbortController for this bundling request
      bundlingAbortRef.current = new AbortController();

      try {
        if (append) setBundlingLoadingMore(true);
        else setBundlingLoading(true);
        ...
        const res = await axiosInstance.get(`/bundlings`, {
          ...
          signal: bundlingAbortRef.current?.signal,
        });
        ...
        setBundlingTotalPages(meta.last_page ?? 1);
        setBundlingHasMore(p < (meta.last_page ?? 1));
  ...
  const fetchProducts = useCallback(
    async (p = 1, append = false) => {
      ...
      if (productAbortRef.current) {
        ...
        productAbortRef.current.abort("New products request initiated");
      }

      // Create new AbortController for this product request
  ...
  useEffect(() => {
    ...
    } else if (noFiltersActive && filter.q) {
      // No filters active and user searched -> show both products and bundlings
      setProducts([]);
      setBundlings([]);
  ...
  const loadMoreBundlings = () => {
    if (!bundlingHasMore || bundlingLoadingMore) return;
    const nextPage = bundlingPage + 1;
                    ...
                    {bundlingLoading ? (
                      ...
                      <>
                        ...
                        {bundlingHasMore && (
                          <div className="mt-8 flex justify-center">
                            <button
                              onClick={loadMoreBundlings}
                              disabled={bundlingLoadingMore}
                              className="rounded-md bg-text-light-primary px-6 py-3 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                            >
                              {bundlingLoadingMore ? "Loading..." : "Load More"}
                            </button>
                          </div>
                        )}
                      </>
