...
export default function Details() {
  ...
  const {
    ...
    queryFn: () => {
      if (process.env.NODE_ENV === "development") {
        console.log("🌐 Details: Fetching product:", {
          slug,
          apiStartDate,
          apiEndDate,
        });
  ...
  const availabilityInfo = useMemo(() => {
    if (!product)
      return { isAvailable: false, quantity: 0, text: "Tidak tersedia" };
  ...
  const handleDateChange = useCallback(
    (newStartDate: string | null, newEndDate: string | null) => {
      if (process.env.NODE_ENV === "development") {
        console.log("🗓️ Details: Date change requested:", {
          newStartDate,
          newEndDate,
        });
      }
  ...
  const seoProps = useMemo(() => {
    ...
    return {
      ...
      description: `Sewa ${product.name} berkualitas tinggi dari ${
        product.brand?.name || "brand terpercaya"
      }. Harga mulai ${formattedPrice}/hari. ${
        product.status === "available"
          ? "Tersedia untuk rental!"
          : "Hubungi kami untuk ketersediaan."
      } Proses mudah dan cepat.`,
      keywords: `rental ${product.name}, sewa ${
        product.category?.name || "kamera"
      }, ${product.brand?.name || ""}, rental kamera ${
        product.category?.name || ""
      } Indonesia, sewa peralatan fotografi`,
      image: product.thumbnail
        ? `${STORAGE_BASE_URL}/${product.thumbnail}`
        : undefined,
      price: product.price,
            ...
            <div className="lg:col-span-1">
              {product && (
                <ProductInfo
                  ...
                  // isLoadingAvailability={isLoadingAvailability}
                  // setIsLoadingAvailability={setIsLoadingAvailability}
  ...
  const mobileBottomBar = useMemo(
    ...
    [formattedPrice, whatsappLink, isAvailable]
  );
        ...
        <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4">
          <div className="text-center">
            ...
            <p className="text-gray-700 mb-6">
              Maaf, terjadi kesalahan saat memuat detail produk. Silakan coba
              lagi nanti.
            </p>
            <div className="space-y-4">
              <button
